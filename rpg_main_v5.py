...
class Personnage() :
    ...
    def Affiche_Caractéristiques(self) :
        ...
        msg1 = f"{self.nom} a une force de {self.force}, une endurance de {self.endurance}, une rapidité de {self.rapidite}, une intelligende de {self.intelligence} et un charisme de {self.charisme}. BG !"
        msg4 = f"{self.nom} est de race : {self.Race} et de classe {self.Role}"
        if self.metier == magicien :
            msg5 = f"son nb de pt de magie est : {self.pointdemagie}"
        else :
            msg5 = ""
        print(msg1)
        print(msg4)
    ...
    def Affiche_etat(self) :
        ...
        self.Est_mort()
        if self.mort == False :
            msg2 = f"{self.nom} a  {self.vie} HP"
        else :
            msg2 = f"{self.nom} est mort. RIP"

    def Perd_vie(self, nb_pv_perdu) :
        '''gère la perte de pv du pers'''
        self.Est_mort()
        if nb_pv_perdu >= self.vie :
            msg = f"{self.nom} succombe des blessures"
            self.mort = True
        else :
            self.vie -= nb_pv_perdu
            msg = f"{self.nom} perd {nb_pv_perdu} HP"
    ...
    def Gain_vie(self, nb_pv_gagne) :
        ...
        if self.mort == False :
            ...
            msg3 = f"{self.nom} récupère {nb_pv_gagne} HP."
        else :
            msg3 = f"{self.nom} ne peut pas être soigné car il est mort."
    ...
    def Soin(self, autre_personnage, Point_de_soin) :
        ...
        if self.mort == False and autre_personnage.mort == False :
            msg6 = f"{self.nom} soigne {autre_personnage.nom} de {Point_de_soin}."
            autre_personnage.Gain_vie(Point_de_soin)
        elif self.mort == True and autre_personnage.mort == False :
            msg6 = f"{self.nom} ne peut pas soigner {autre_personnage.nom} car {self.nom} est mort"
        elif self.mort == False and autre_personnage.mort == True :
            msg6 = f"{self.nom} ne peut pas soigner {autre_personnage.nom} car {autre_personnage.nom} est mort"
        else :
            msg6 = f"{self.nom} et {autre_personnage.nom} sont morts ! Pas de soin possible"
    ...
    def Esquive_attaque(self, autre_personnage) :
        ...
        if round(self.rapidite * 1.2) > autre_personnage.force :
            self.esquive = True
            msg7 = f"{autre_personnage.nom} rate son attaque sur {self.nom}"
            print(msg7)
        else :
            self.esquive = False
            msg7 = f"{autre_personnage.nom} attaque {self.nom}"
            print(msg7)
    ...
    def Attaque(self, autre_personnage) :
        ...
        if self.mort == False and autre_personnage.mort == False :
            ...
            msg8 ="Attaque impossible un des deux personnages est mort."
